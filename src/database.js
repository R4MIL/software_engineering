...
const saveHistory = (city,data) => {
    return new Promise(function(reject) {
        pool.query('INSERT INTO history_query (city,lat,lon,pollution_data,datetime) VALUES ($1, $2, $3, $4, $5)', [city,data.coord.lat,data.coord.lon,data.list[0],new Date().toLocaleString()], (error) => {
        if (error) {
            reject(error)
        }
      })
